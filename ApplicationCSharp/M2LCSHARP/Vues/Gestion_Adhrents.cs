    ...
    public partial class Gestion_Adherents : Form {
        ...
        private void Gestion_Adherents_Load(object sender, EventArgs e) {
            ...
            foreach (var item in gAdhe.liste) {
                ...
                try { dataGridView1.Rows.Add(item.Id, item.Nom, item.Prenom, item.CodePostal, item.Ville, item.Adresse, item.numero_licence, item.Cotisation, item.club.Titre_club); }
                catch { dataGridView1.Rows.Add(item.Id, item.Nom, item.Prenom, item.CodePostal, item.Ville, item.Adresse, "Aucun", "Aucune", "Pas de club"); }
